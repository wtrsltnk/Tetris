...
void Block::renderBlock(bool useColor)
{
    ...
    for (int x = 0; x < this->mSize; x++)
    {
        for (int y = 0; y < this->mSize; y++)
        {
            ...
            {
                if (useColor)
                    glColor4ub(this->mSubBlocks[x][y] & 0xff,
                               (this->mSubBlocks[x][y] >> 8) & 0xff,
                               (this->mSubBlocks[x][y] >> 16) & 0xff,
                               (this->mSubBlocks[x][y] >> 24) & 0xff);
...
#define MAX_BLOCK_ROW 64
#define MAX_BLOCK_COL 64

void Block::rotate()
{
    unsigned int tmp[MAX_BLOCK_ROW][MAX_BLOCK_COL];
        ...
        for (int y = 0; y < this->mSize; y++)
        {
            if (this->mSubBlocks[x][y] != 0)
            {
                ...
                if (this->mX + x + addX >= FIELD_WIDTH)
                    return false;
                if (this->mField->isBlock(this->mX + x + addX, this->mY + y + addY) !=
                    makeColor(0, 0, 0, 0))
                    return false;
            }
        }
